export function DATAMatrix(Q) {
  ...
  const toEdifact = function(t) {
    const
      n = t.length
    const l = (n + 1) & -4; let cw = 0; let ch
    const r = (l > 0) ? [240] : [] /* switch to Edifact */
  ...
  const toText = function(t, s) {
    let
      i; let j
    let cc = 0
    let cw = 0
    const l = t.length
    let r = [s[0]] /* start switch */
    ...
    for (i = 0; i < l; i++) {
      ...
      if (ch > 127 && r[0] !== 238) { /* extended char */
        push(1),
        push(30),
  ...
  const encodeMsg = function(text, rct) {
    ...
    let
      h; let w; let nc = 1; let nr = 1; let fw; let fh /* symbol size, regions, region size */
    let i; let j = -1; let c; let r; let s; let b = 1 /* compute symbol size */

    const rs = new Array(70) /* reed / solomon code */
    const rc = new Array(70)
    const lg = new Array(256) /* log / exp table for multiplication */
    ...
    for (rs[s] = 0, i = 1; i <= s; i++) {
      for (j = s - i, rs[j] = 1; j < s; j++) { rs[j] = rs[j + 1] ^ ex[(lg[rs[j]] + i) % 255] }
    }
    ...
    for (c = 0; c < b; c++) {
      ...
      for (i = c; i < el; i += b) {
        for (j = 0, x = rc[0] ^ enc[i]; j < s; j++) { rc[j] = rc[j + 1] ^ (x ? ex[(lg[rs[j]] + lg[x]) % 255] : 0) }
      }
  ...
  return (function() {
    ...
    const
      abs = Math.abs
    let r; let x; let y; let d; let sx; let sy

    var ns = 'http://www.w3.org/2000/svg'
    let path = ''

    const q = (typeof Q === 'string') ? { msg: Q } : Q || {}
    const p = q.pal || ['#000']
    const dm = abs(q.dim) || 256
    var pd = abs(q.pad); var pd = (pd > -1) ? pd : 2
    const mx = [1, 0, 0, 1, pd, pd]

    var fg = p[0]; var fg = ishex(fg) ? fg : '#000'
    var bg = p[1]; var bg = ishex(bg) ? bg : 0

    /* render optimized or verbose svg */
    const optimized = (q.vrb) ? 0 : 1
