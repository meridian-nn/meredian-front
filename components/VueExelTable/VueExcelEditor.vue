        ...
        >
          ...
          <tbody @mousedown="mouseDown">
            ...
            >
              <td
                ...
                @click="rowLabelClick"
              >
                <span v-html="recordLabel(pageTop + rowPos + 1, record)" />
              </td>
          ...
          <tfoot>
            <tr v-show="pagingTable.length && summaryRow">
              <td class="row-summary first-col">
&nbsp;
              </td>
      ...
      >
        ...
        >
          <span v-html="localizedLabel.footerLeft(pageTop + 1, pageBottom, table.length)" />
        </span>
...
export default {
  ...
  computed: {
    ...
    pageBottom() {
      if (this.filteredValue.length === 0) { return 0 } else { return this.pageTop + this.pageSize > this.table.length ? this.table.length : this.pageTop + this.pageSize }
    },
    setting: {
      get() {
        return null
  ...
  methods: {
    ...
    calTable() {
      ...
      if (this.showFilteredOnly === false) {
        ...
        if (filterColumnList.length === 0) { this.table = this.filteredValue } else {
          this.table = this.filteredValue.filter((record) => {
            ...
            */
            ...
            filterColumnList.forEach((k) => {
              ...
              if (this.fields[k].type === 'number' && filter[k].type <= 4) { content[k] = val } else { content[k] = typeof val === 'undefined' || val === null ? '' : String(val).toUpperCase() }
            })
    ...
    clearFilter(name) {
      if (!name) { this.columnFilter = {} } else { this.setFilter(name, '') }
    },
    ...
    vsbMouseMove(e) {
      if (e.buttons === 0) { this.vsbMouseUp() } else {
        ...
        if (this.noPaging) {
          ...
          this.vScroller.runner = recPos + '<br>' + this.fields
            ...
            .join('<br>')
          console.log(this.vScroller.runner)
          this.$forceUpdate()
        }
      }
    },
    ...
    sbMouseMove(e) {
      if (e.buttons === 0) { this.sbMouseUp() } else {
    ...
    mousewheel(e) {
      ...
      let adjust = 0
      if (e.deltaY > 30 && this.pageTop + this.pageSize < this.table.length) { adjust = 1 } else if (e.deltaY < -30 && this.pageTop > 0) { adjust = -1 }
      if (adjust) {
        this.pageTop += adjust
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        switch (e.keyCode) {
          ...
          case 9: // Tab
            ...
            if (e.shiftKey) {
              if (!this.moveWest(e)) {
                if (this.moveNorth(e)) { this.moveToEast(e) } else { return this.inputBoxBlur() }
              }
            } else if (!this.moveEast(e)) {
          ...
          default:
            ...
            if (this.currentField.allowKeys) {
              ...
              if (!this.currentField.allowKeys.includes(e.key.toUpperCase())) { return e.preventDefault() }
            }
    ...
    doFind(s) {
      ...
      for (let r = row + this.pageTop; r < this.table.length; r++) {
        ...
        for (let c = (r === row + this.pageTop ? this.currentColPos + 1 : 0); c < this.fields.length; c++) {
          ...
          if (typeof rec[field] !== 'undefined' && String(rec[field]).toUpperCase().includes(s)) {
    ...
    panelFilterClick(item) {
      if (!this.disablePanelFilter) { this.$refs.panelFilter.showPanel(this.$refs[`filter-${item.name}`][0]) }
    },
            ...
            while (pass < 2) {
              ...
              await Promise.all(importData.map(async(line, i) => {
                ...
                await Promise.all(this.fields.map(async(field) => {
                  ...
                  if (val !== null) { rec[field.name] = val } else if (field.mandatory) {
    ...
    exportTable(format, selectedOnly, filename) {
      ...
      setTimeout(() => {
        ...
        const labels = Array.from(this.labelTr.children).slice(1).map(t => t.children[0].textContent)
    ...
    */
    ...
    clearAllSelected() {
      ...
      if (this.selectedCount > 0) { this.$emit('select', Object.keys(this.selected).map(rowPos => Number(rowPos)), false) }
      this.selected = {}
      this.selectedCount = 0
    },
    ...
    moveToSouthWest() {
      const goRowPos = this.table.length - 1
      let goColPos = 0
      while (this.fields[goColPos].invisible && goColPos < this.fields.length - 1) { goColPos++ }
    ...
    moveToWest() {
      const goRowPos = this.currentRowPos
      let goColPos = 0
      while (this.fields[goColPos].invisible && goColPos < this.fields.length - 1) { goColPos++ }
      return this.moveTo(goRowPos, goColPos)
    },
    ...
    cellMouseMove(e) {
      ...
      if (!e.target.classList.contains('readonly') &&
        ...
        e.target.offsetWidth - e.offsetX < 15) { cursor = 'pointer' }
    ...
    moveInputSquare(rowPos, colPos) {
      ...
      if (!row) {
        if (rowPos > this.currentRowPos) {
          ...
          if (this.pageTop + this.pageSize < this.table.length) { this.pageTop += 1 } else { return false }
          row = this.recordBody.children[--rowPos]
        } else {
          // move the whole page up 1 record
          if (this.pageTop - 1 >= 0) { this.pageTop -= 1 } else { return false }
          row = this.recordBody.children[++rowPos]
        }
      }
    ...
    inputCellWrite(setText, colPos, recPos) {
      ...
      if (typeof this.selected[recPos] !== 'undefined') { this.updateSelectedRows(field, field.toValue(setText)) } else { this.updateCell(recPos, field, field.toValue(setText)) }
    },
    inputBoxBlur() {
      if (!this.$refs.dpContainer) { return }
    ...
    newRecord(rec, selectAfterDone, noLastPage, isUndo) {
      ...
      this.fields.map((f) => {
        if (typeof rec[f.name] === 'undefined') {
          if (f.keyField) { rec[f.name] = '§' + this.tempKey() } else { rec[f.name] = null }
        }
      })
        ...
        const doList = async() => {
          ...
          if (field.options) {
            if (field.options.constructor.name.endsWith('Function')) {
              ...
              if (field.type === 'map') { list = Object.values(list) } else { list = list.slice() }
            ...
            for (let i = 0; i < this.value.length; i++) {
              ...
              if (typeof rec[name] !== 'undefined' && rec[name].toString().toUpperCase().startsWith(value) && !list.includes(rec[name])) { list.push(rec[name]) }
          ...
          this.lazy(() => {
            ...
            if (rect.left + r.width > window.innerWidth) { this.$refs.autocomplete.style.top = (window.innerWidth - r.width) + 'px' } else { this.$refs.autocomplete.style.left = rect.left + 'px' }
            const showTop = this.autocompleteSelect * 23 - 206
            this.$refs.autocomplete.scrollTop = showTop > 0 ? showTop : 0
          })
          return this.autocompleteSelect
        }
        if (force) { doList() } else { this.lazy(doList, 700) }
      }
    },
